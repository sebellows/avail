...
import { isPlainObject } from '../../../core/src'
...
export const resolveDir = async (path: string): Promise<string> => {
  const dirPath = path.includes(ROOT_PATH) ? path : join(ROOT_PATH, path)
  ...
  try {
    ...
    if (path === '.') {
      logger.error(`Provided path points to Indigo's root directory ("${path}").`)
      process.exit(1)
    }
...
export const createFile = async (
  ...
  try {
    ...
    setTimeout(() => {
      handleBuildError('Could not write command file to bin/')
    })
  }
}
