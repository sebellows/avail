 ...
 *
 * @description
 * Returns a controller object for requesting and cancelling an animation freame
 * that is properly cleaned up once the component unmounts. New requests cancel
 * and replace existing ones.
 *
 * ```ts
 * const [style, setStyle] = useState({});
 * const animationFrame = useRAF();
 *
 ...
 * ```
 */
export function useRAF(): UseAnimationFrameReturn {
  ...
  return useStableMemo(
    () => ({
      request(cancelPrevious: boolean | FrameRequestCallback, fn?: FrameRequestCallback) {
        ...
        handle.current = requestAnimationFrame(fn || (cancelPrevious as FrameRequestCallback));
      },
      cancel,
    }),
    [],
