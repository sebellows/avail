...
import { mixin, color, transition } from '../../core/utils';
...
export const StyledToggleControl = styled(Control)`
  opacity: 0;
  z-index: 1;
  ${mixin.cover}
  ...
  &:checked ~ .toggle-container .toggle-outer {
    ...
    border-color: ${(props: ToggleControlProps) =>
      props?.type === 'radio' ? color.primary : color.control.border};
  }
...
`;
console.log('StyledToggleControl', StyledToggleControl);
...
`;
export const ToggleInner = styled(ToggleOuter)`
